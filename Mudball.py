...
####################################################################################################
    # define and set logging
logger = logging.getLogger('Mudball')
logger.setLevel(logging.DEBUG)
format = logging.Formatter('%(asctime)s  %(name)s--%(levelname)s : %(message)s')

fh = logging.FileHandler('Mudballlog_log.txt')
...
def get_user_input(name):
    ...
    try:
        ...
        logger.debug(name + ' : PSI = {},angle = {}'.format(psi,angle))
        return psi, angle
    except ValueError:
        logger.error('should enter number but not string')
        get_user_input(name)
        return 0,0
...
def process_player_turn(player_name, distance_apart):
    ...
    if difference > 1:
        ...
        logger.debug(str(player_name) + str(difference) + "yards too short")
    elif difference < -1:
        print("You were", difference * -1, "yards too short!")
        logger.debug(str(player_name) + str(difference * -1) + "yards too short!")
    else:
        print("Hit!", player_name, "wins!")
        logger.debug(str(player_name) + "wins!")
        return True
...
def main():
    ...
    logger.debug('----------The programme is start.----------')
    print_instructions()
    logger.debug(print_instructions())
    player_names = get_player_names()
    distance_apart = random.randrange(50, 150)
    logger.debug('distance = {}'.format(distance_apart))
